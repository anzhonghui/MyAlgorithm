 ...
 * 　　　┗┻┛　┗┻┛
 *
 * @Description : 判断数独是否是有效数独，满足每行是9个数字，每列是9个数字，每个3*3是9个数字
 * @Programme：利用set集合存储不重复值的特性，需要注意九宫格的计算方式
 * 确定[i][j]（i是行，j是列）所在的九宫格编号，然后前面乘以3，得到对应九宫格左上角的坐标，通过计算添加到集合中某一个块的值
...
public class ValidSudoku {
	 ...
	 * 跟下面的方法思路类似，只是将数据存储到三个set集合中，一个存储行的数字，一个存储列的数字，一个存储3*3的数字
	 * @param board
	 * @return
	 */
